function Get-GraphDeviceCompliancePolicy() {
    <#
        .SYNOPSIS
            Get Intune Device Compliance Policies through Microsoft Graph
        .DESCRIPTION
            https://docs.microsoft.com/en-us/graph/api/intune-deviceconfig-devicecompliancepolicy-get?view=graph-rest-beta
            https://docs.microsoft.com/en-us/graph/api/intune-deviceconfig-devicecompliancepolicy-list?view=graph-rest-beta
        .PARAMETER DeviceCompliancePolicyId
            If exists, a single device configuration matching DeviceCompliancePolicyId will be returned
        .PARAMETER All
    ...
    process {
        try {
            ...
            $uri = "https://graph.microsoft.com/$apiVersion/deviceManagement/deviceCompliancePolicies?`$top=999"
            $query = Invoke-RestMethod -Uri $uri -Headers $authHeader -Method Get -ErrorAction Stop
            $value = $query.Value
