function Get-GraphDeviceManagementIntent() {
    <#
        .SYNOPSIS
            Get Intune Device Management Intents through Microsoft Graph
        .DESCRIPTION
            https://docs.microsoft.com/en-us/graph/api/intune-deviceintent-devicemanagementintent-get?view=graph-rest-beta
            https://docs.microsoft.com/en-us/graph/api/intune-deviceintent-devicemanagementintent-list?view=graph-rest-beta
        .PARAMETER Id
            If exists, a single device management intent matching the given id will be returned
        .PARAMETER All
            If set to $true, all device management intent will be returned. By default this is limited to 999 (Microsoft Graph Paging is limited to 999 results)
    ...
    process {
        try {
            ...
            # Return one (GET)
            if ($id) {
                $uri = "https://graph.microsoft.com/$apiVersion/deviceManagement/intents/$($id)"
            ...
            $uri = "https://graph.microsoft.com/$apiVersion/deviceManagement/intents?`$top=999"
            $query = Invoke-RestMethod -Uri $uri -Headers $authHeader -Method Get -ErrorAction Stop
            $value = $query.Value
