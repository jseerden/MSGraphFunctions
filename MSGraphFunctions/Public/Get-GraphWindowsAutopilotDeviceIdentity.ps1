function Get-GraphWindowsAutopilotDeviceIdentity() {
    <#
        .SYNOPSIS
            Get Windows Autopilot Devices through Microsoft Graph
        .DESCRIPTION
            https://docs.microsoft.com/en-us/graph/api/intune-enrollment-windowsautopilotdeviceidentity-list?view=graph-rest-beta
            https://docs.microsoft.com/en-us/graph/api/intune-enrollment-windowsautopilotdeviceidentity-get?view=graph-rest-beta
        .PARAMETER Id
            If exists, a single Windows Autopilot Device matching Id will be returned
        .PARAMETER All
    ...
    process {
        try {
            ...
            # Return one (GET)
            if ($id) {
                $uri = "https://graph.microsoft.com/$apiVersion/deviceManagement/windowsAutopilotDeviceIdentities/$($id)"
            ...
            $uri = "https://graph.microsoft.com/$apiVersion/deviceManagement/windowsAutopilotDeviceIdentities?`$top=999"
            $query = Invoke-RestMethod -Uri $uri -Headers $authHeader -Method Get -ErrorAction Stop
            $value = $query.Value
