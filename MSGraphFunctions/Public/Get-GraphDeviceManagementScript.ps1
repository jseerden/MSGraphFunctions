function Get-GraphDeviceManagementScript() {
    <#
        .SYNOPSIS
            Get Intune Device Management Scripts through Microsoft Graph
        .DESCRIPTION
            https://docs.microsoft.com/en-us/graph/api/intune-devices-devicemanagementscript-get?view=graph-rest-beta
            https://docs.microsoft.com/en-us/graph/api/intune-devices-devicemanagementscript-list?view=graph-rest-beta
        .PARAMETER Id
            If exists, a single device management script matching the given id will be returned
        .PARAMETER All
            If set to $true, all device management scripts will be returned. By default this is limited to 999 (Microsoft Graph Paging is limited to 999 results)
    ...
    process {
        try {
            ...
            $uri = "https://graph.microsoft.com/$apiVersion/deviceManagement/deviceManagementScripts?`$top=999"
            $query = Invoke-RestMethod -Uri $uri -Headers $authHeader -Method Get -ErrorAction Stop
            $value = $query.Value
