function Get-GraphGroupPolicyConfiguration() {
    <#
        .SYNOPSIS
            Get Intune Group Policy Configurations through Microsoft Graph
        .DESCRIPTION
            https://docs.microsoft.com/en-us/graph/api/intune-grouppolicy-grouppolicyconfiguration-list?view=graph-rest-beta
            https://docs.microsoft.com/en-us/graph/api/intune-grouppolicy-grouppolicyconfiguration-get?view=graph-rest-beta
        .PARAMETER Id
            If exists, a single group policy configuration matching Id will be returned
        .PARAMETER All
            If set to $true, all group policy configurations will be returned. By default this is limited to 999 (Microsoft Graph Paging is limited to 999 results)
    ...
    process {
        try {
            ...
            $uri = "https://graph.microsoft.com/$apiVersion/deviceManagement/groupPolicyConfigurations?`$top=999"
            $query = Invoke-RestMethod -Uri $uri -Headers $authHeader -Method Get -ErrorAction Stop
            $value = $query.Value
